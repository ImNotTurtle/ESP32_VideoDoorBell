...
void IRAM_ATTR interruptHandle()
{
  BaseType_t xHigherPriorityTaskWoken = pdFALSE;
  xSemaphoreGiveFromISR(interruptTriggerSem, &xHigherPriorityTaskWoken);
...
void setup()
{
  ...
  if (connected)
  {
    ...
    attachInterrupt(digitalPinToInterrupt(BUTTON_PIN), interruptHandle, LOW);
    interruptTriggerSem = xSemaphoreCreateBinary();
    xTaskCreate([](void *arg)
                {
          vTaskDelay(pdMS_TO_TICKS(5000));
          while(true){
            if(xSemaphoreTake(interruptTriggerSem, portMAX_DELAY)){
              Serial.println("BroadcastBell");
              WebSocket_Module::BroadcastBell();
            }
          } }, "ButtonHandleTask", 1024 * 2, NULL, 5, NULL);
  }
}
